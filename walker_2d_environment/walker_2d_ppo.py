...
class PPOSolver:
    def __init__(self):
        ...
        self.n_mini_batch = 4
        self.epochs = 30
        self.save_directory = "./walker_2d_ppo"
        self.batch_size = self.worker_steps
        self.mini_batch_size = self.batch_size // self.n_mini_batch
        self.obs = env.reset()
        self.policy = Model(env.action_space.shape[0], 0.6).to(device)
        self.mse_loss = nn.MSELoss()
        self.optimizer = torch.optim.Adam([
            {'params': self.policy.actor.parameters(), 'lr': 0.0005},
            {'params': self.policy.critic.parameters(), 'lr': 0.001}
        ])
        self.policy_old = Model(env.action_space.shape[0], 0.6).to(device)
    ...
    def sample(self):
        ...
        obs = np.zeros((self.worker_steps, env.observation_space.shape[0]), dtype=np.float32)
